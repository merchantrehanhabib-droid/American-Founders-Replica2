...
export default function Hero() {
  ...
  return (
    ...
    >
      ...
      {/* Decorative stars */}
      <div className="absolute top-20 left-10 text-gold/25 animate-pulse z-10">
        <Star className="w-10 h-10 fill-current" />
      </div>
      <div className="absolute bottom-16 left-1/3 text-gold/15 animate-pulse delay-700 z-10">
        <Star className="w-7 h-7 fill-current" />
      </div>
      <div className="absolute top-1/2 right-[55%] text-gold/10 animate-pulse delay-1000 z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-10 items-start">
          ...
          <div className="space-y-7 pt-4">
            ...
            <div className="flex flex-wrap gap-3">
              <div className="flex items-center gap-3 bg-white/10 rounded-lg px-4 py-3 border border-white/10 backdrop-blur-sm">
            ...
            <div className="bg-white rounded-2xl shadow-2xl overflow-hidden">
              ...
              <div className="px-8 py-7">
                {submitted ? (
                  ...
                  <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                      ...
                      <input type="text" required placeholder="Enter your full name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition" />
                    </div>
                    <div>
                      <label className="block text-navy font-semibold text-sm mb-1.5">Email Address <span className="text-red-500">*</span></label>
                      <input type="email" required placeholder="Enter your email address" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition" />
                    </div>
                    <div>
                      <label className="block text-navy font-semibold text-sm mb-1.5">Phone Number</label>
                      <input type="tel" placeholder="Enter your phone number" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full border border-gray-200 rounded-lg px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-gold focus:ring-2 focus:ring-gold/20 transition" />
                    </div>
                    <div>
                    ...
                    <button type="submit" className="w-full bg-navy hover:bg-navy/90 text-white font-bold py-4 rounded-lg text-base transition-colors mt-2">
                      Get A Free Consultation
                    </button>
                  </form>
                )}
                ...
                <div className="mt-6 pt-5 border-t border-gray-100 flex flex-col sm:flex-row gap-3 justify-center items-center text-sm text-gray-500">
                  <span className="flex items-center gap-1.5"><Phone className="w-4 h-4 text-gold" />+1 (800) 555-0199</span>
                  <span className="hidden sm:block text-gray-200">|</span>
                  <span className="flex items-center gap-1.5"><Mail className="w-4 h-4 text-gold" />info@americanbookfounders.com</span>
                </div>
              </div>
            </div>
          </div>
